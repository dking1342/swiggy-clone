<template>
    <section v-if="!orderList.order_item.length" class="cart-empty-container">
    ...
    <section v-else-if="orderList.order_item.length && !isPaymentModal" class="cart-fill-container ">
        <div class="account-container cart-container view-containers">
            ...
            <button v-if="!isFormShowing && !loginStatus" @click="isFormShowing = true" class="account-login account-btn"><span>Want your food?</span><span>Log In</span></button>
            ...
            <div class="checkout-login-container" v-if="isFormShowing || loginStatus">
                <Forms />
            </div>
        </div>
        <div class="delivery-container cart-container view-containers">
            <h1>Delivery location</h1>
            <div class="delivery-address-container" v-if="loginStatus">
                <address>
        ...
        <div class="payment-container cart-container view-containers">
            <h1>Payment</h1>
            <button v-if="loginStatus" class="cc-btn" @click="goToPaymentModal"><span>Pay with</span><span>Credit Cart</span></button>
            <button v-if="loginStatus" class="paypal-btn" @click="goToPaymentModal"><span>Pay with</span><span>Paypal</span></button>
        ...
        <div class="order-container view-containers">
            ...
            <div class="order-item-container">
                <div class="order-item" v-for="item in orderList.order_item" :key="item.order_id">
                    ...
                    <div class="order-btn-container">
                        ...
                        <button class="add-order" @click="addToCart(item.order_item_name.menu_id)">+</button>
                    </div>
            ...
            <div class="order-totals-container">
                ...
                <div class="totals-row">
                    ...
                    <p>${{ (orderList.cart_cost * .07).toFixed(2) }}</p>
                </div>
                <div class="totals-row">
                    <p>To Pay</p>
                    <p>${{ (orderList.cart_cost + (orderList.cart_cost * 1.07 ) + 50).toFixed(2)}}</p>
                </div>
            </div>
        </div>
    </section>
    <section class="payment-modal" v-else-if="orderList.order_item.length && isPaymentModal">
        <div class="pay-container" v-if="!isPaymentProcessing">
            <button class="close-payment-btn" @click="closePaymentModal">X</button>
            <div class="checkout-header">
                <h1>Checkout</h1>
                <h2>${{ (orderList.cart_cost + (orderList.cart_cost * 1.07 ) + 50).toFixed(2) }}</h2>
            </div>
...
<script lang="ts">
import { DATASTATE, Discount, FetchResponse, Menu, OrderConfirmationItem, OrderReceipt, Restaurant } from '@/types/fetch-types';
import { computed, defineComponent, onMounted, ref } from 'vue'
import { useRouter } from 'vue-router';
import Forms from './Forms.vue';
import { useStore } from 'vuex';
import { useFetch, useAPICall } from '@/utils/useFetch';
...
export default defineComponent({
    ...
    setup () {
        ...
        const orderList = computed(()=> store.state.cart.order);
        const userLocation = computed(() => store.state.location.state.location);

        // fetches
        const getRestaurant = async(id:any) => {
            let { data } = await useFetch<FetchResponse<Restaurant<Discount>>>(`http://localhost:8000/api/v1/restaurants/${id}`);
            let restaurantState:any = data.value;
            restaurant.value = restaurantState.data[0];
        }
        const createOrder = async(order:any) => {
            const { dataState, data } = await useAPICall<FetchResponse<OrderConfirmationItem>>('http://localhost:8000/api/v1/cart/create/',order,"POST");            
            const id = data.value.data[0].order_reference;

            if(dataState.value === DATASTATE.success){
                isPaymentProcessing.value = true;

                // remove cart from local storage
                store.dispatch('cart/removeCartAction');

                // success to confirmation view
                setTimeout(()=>{
                    router.push({name:"Restaurants"})
                    router.push({name:"Confirmation",params:{id}})
                },3000);
            } else if (dataState.value === DATASTATE.error){
        ...
        onMounted(()=>{
            ...
            let id = orderList.value.order_item[0]?.order_item_name.restaurant;
            if(id){
                getRestaurant(id);
            }
...
<style scoped>
    ...
    .account-container h1:first-child{
        grid-area: h;
        font-size: 32px;
    }
    .account-container h1:last-child{
        font-size: 24px;
    }
    .account-container p{
        grid-area: p;
